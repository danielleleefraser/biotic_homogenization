...
#### Steps to re-producing Figures and Analyses######################################################################################

source("Mega.beta.diversity function_Oct2021.r")
source("Accessory functions_Oct2021.r")

###########################################################################################################################################
###########################################################################################################################################

# Calculate Jaccard similarity for the various data subsets and for the null model
# Calculate range size and occupancy for the various data subsets and for the null model
...
# These are placeholders used but the date ranges are changed appropriately when plotting
age_bins<-seq(from=0,to=maxi,by=5000)
age_bins<-c(500,age_bins)

# Alternative breaks in the ages so as to align with the end of Heinrich Stadial 1
#age_bins<-c(0,500,4200,11700,17000,25000,32500)

# Jaccard dissimilarity (converted downstream to similarity)
age_bins<-age_bins[1:8] # don't run with the alternative breaks
...
Null_final<-cbind(Jaccard_all[[1]][,1],test_1)
...
# All species without rocky mountains #########################################################################################
##############################################################################################################################################
...
# Jaccard dissimilarity (converted to similarity downstream)
...
# Alternative breaks in the ages as suggested by reviewers
#age_bins<-c(0,500,4200,11700,17000,25000,32500)
...
ggplot(df2, aes(x=Age7,y=Jaccard_alaska))+
  ...
  scale_x_reverse(breaks=c(30000,25000,20000,15000,10000,5000,0),
                  labels=c("30 ka-25 ka","25 ka-20 ka","20 ka-15 ka","15 ka-10 ka","10 ka - 5 ka","5 ka - modern","Modern"))+  
  ylim(0.0,0.4)+
  xlab("Time bins")+
...
##### Alternative Figure 1 (Figure S3A)###################################################################################################
###########################################################################################################################
...
# ggplot(df2, aes(x=Age7,y=Jaccard_alaska))+
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none")+
#   scale_x_reverse(breaks=c(25000,17000,11700,4200,0),
#                   labels=c("25 ka - 17 ka","17 ka-11.7 ka","11.7 ka - 4.2 ka","4.2 ka-0.5 ka","Modern"))+  
#   ylim(0.0,0.4)+
#   xlab("Time bins")+
#   ylab("Mean proportion of shared taxa")+
#   geom_ribbon(aes(ymin=Lower,ymax=Upper),fill="grey70")+
...
# This part of the code produces the brown solid and dotted lines in Figure 2
...
sites_ages<-sites_ages[matches,]
latlongs<-latlongs[matches,]
Agetable<-cbind(sites_ages,latlongs)
Agetable$sitekey<-NULL
Agetable$sitekey<-NULL
head(Agetable)
Agetable$time.bins <- cut(Agetable$timeybp, breaks=c(0,500,5000,10000,15000,20000,25000,30000), labels=c(0,5000,10000,15000,20000,25000,30000) )
head(Agetable)
# Alternative breaks
# Agetable$time.bins <- cut(Agetable$timeybp, breaks=c(0,500,4200,11700,17000,25000,30000), labels=c(0,4200,11700,17000,25000,30000))
# head(Agetable)
...
for(p in 1:length(climvars)){
  ...
  for(i in 1:(length(ages)-1)){
    ...
    upper<-ages[i+1] 
    lower<-ages[i]
    sites_age_new<-Agetable[Agetable$timeybp<=upper,]
...
##### Alternative Figure 2 (Figure S3B)###################################################################################################
###########################################################################################################################
...
#   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none")+
#   scale_x_reverse(breaks=c(25000,17000,11700,4200,0),
#                   labels=c("25 ka - 17 ka","17 ka-14.7 ka","11.7 ka - 4.2 ka","4.2 ka-0.5 ka","Modern"))+
#   ylim(0.0,3.0)+
#   xlab("Time bins")+
#   ylab("Mean climate difference")+
#   geom_point(aes(x=Age,y=Mean_PC1),colour="black")+
...
# This part of the code produces the orange solid and dotted lines in Figure 2
...
lats<-seq(min_lat,max_lat, by=0.25)
longs<-seq(min_long,max_long,length.out=length(lats))
coordinates<-data.frame(x=longs,y=lats)
coord<-SpatialPoints(coordinates, proj4string=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
...
for(p in 1:length(climvars)){
  ...
  names(climate.raster)<-paste(name)
  results<-list()
  for(i in 1:length(climate.raster)){
    print(i)
    results[[i]]<-extract(climate.raster[[i]],coord)
  }
  names(results)<-names(climate.raster)
  all_results[[p]]<-results
}
names(all_results)<-climvars


temp<-all_results[[1]]
temp2<-data.frame(temp[[1]])
colnames(temp2)<-climvars[1]
temp_names<-data.frame(rep(names(temp)[1],nrow(temp2)))
colnames(temp_names)<-c("Ages")
for(p in 2:length(temp)){
  print(p)
  temp3<-data.frame(temp[[p]])
  colnames(temp3)<-climvars[1]
  temp2<-rbind(temp2,temp3)
  temp_names2<-data.frame(rep(names(temp)[p],nrow(temp3)))
  colnames(temp_names2)<-c("Ages")
  temp_names<-rbind(temp_names,temp_names2)
}

final_results<-list()
final_results[[1]]<-temp2
final_names<-list()
final_names[[1]]<-temp_names
for(j in 2:length(climvars)){
  temp<-all_results[[j]]
  temp2<-data.frame(temp[[1]])
  colnames(temp2)<-climvars[j]
  temp_names<-data.frame(rep(names(temp)[1],nrow(temp2)))
  colnames(temp_names)<-c("Ages")
  for(p in 2:length(temp)){
    print(p)
    temp3<-data.frame(temp[[p]])
    colnames(temp3)<-climvars[j]
    temp2<-rbind(temp2,temp3)
    temp_names2<-data.frame(rep(names(temp)[p],nrow(temp3)))
    colnames(temp_names2)<-c("Ages")
    temp_names<-rbind(temp_names,temp_names2)
  }
  final_results[[j]]<-temp2
  final_names[[j]]<-temp_names
}
names(final_results)<-climvars

library(rlist)
all_bound<-list.cbind(final_results)
colnames(all_bound)<-climvars
all_names<-list.cbind(final_names)
colnames(all_names)<-climvars
all_data<-cbind(all_names[,1],all_bound)
all_data<-na.omit(all_data)

PCA_results<-prcomp(~an_avg_TMAX.tif+an_avg_TMIN.tif+an_sum_AET.tif+an_sum_PRCP.tif,data=all_data,scale.=TRUE)
loadings<-as.data.frame(PCA_results$rotation)
biplot(PCA_results)
PCA_results<-PCA_results$x
PCA_results<-cbind(all_data[,1],PCA_results)
colnames(PCA_results)[1]<-c("Age")
PCA_results<-data.frame(PCA_results)

summarized_results<-Climate_PCA_turnover_hiRes(PCA_results)
colnames(summarized_results)<-c("Age","Mean_PC1","SE_PC1","Mean_PC2","SE_PC2")
  
library(dplyr)
forPlot <- data.frame(summarized_results)
# Alternative time bins
df2 <- mutate_all(forPlot, function(x) as.numeric(as.character(x)))

library(ggplot2)
ggplot(df2, aes(x=Age,y=Mean_PC1))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none")+
  scale_x_reverse(breaks=df2$Age[order(df2$Age)],labels=df2$Age[order(df2$Age)])+  
  ylim(0.0,4.0)+
  xlab("Time bins")+
  ylab("Mean climate difference")+
  geom_point(aes(x=Age,y=Mean_PC1),colour="black")+
...
#######Calculating values for Figure S5#####################################################################################################
#########################################################################################################################
...
#######Calculating values for Figure S7#####################################################################################################
#########################################################################################################################
...
# The warnings here are not a problem
...
plot(Forb[[2]][,3],as.numeric(Forb[[1]][,2]),ylim=c(0.1,0.5))
model<-lm(as.numeric(Forb[[1]][,2])~Forb[[2]][,3])
summary(model)
abline(model)

# Panel B

plot(DDecay[[2]][,2],DDecay[[1]][,2])
model2<-lm(DDecay[[1]][,2]~DDecay[[2]][,2])
summary(model2)
abline(model2)
...
sites$time.bins <- cut(sites$timeybp, breaks=c(0,500,5000,10000,15000,20000,25000,32500), labels=c("0-25","500-5000","5000-10000","10000-15000","15000-20000","20000-25000","25000-30000"))
...
for(i in 1:num.age){
  ...
  p<-ggplot() + 
    ...
    geom_point(data = new, aes(longitude, latitude), size=4, color="red", position = position_jitter(w = 0.3, h = 0.5)) +
